    ...
    window.patchBundle = function(src){
      ...
      try {
        // DIAGNOSTIC BRIDGE: expose game-realm internals + party/class driving + Gloo Wall.
             var bridge = ";function __walk(o,fn){if(!o)return;fn(o);if(o.children){for(var i=0;i<o.children.length;i++)__walk(o.children[i],fn);}};var _glooModel=null;try{if(typeof XE==='function'){var _ldr=XE(!![]);_ldr.load('models/gloo_wall.glb',function(g){if(g&&(g.scene||g.scenes)){var rawModel=g.scene||g.scenes[0];_glooModel=rawModel;window._glooModel=_glooModel;console.log('[GLOO] Loaded custom Free Fire gloo_wall.glb 3D model (raw natural pivot)!');}},undefined,function(err){console.error('[GLOO] load err:',err);});}}catch(e){};function __mkGlooGeom(T){var BG=T.BufferGeometry,FA=T.Float32BufferAttribute;if(!BG||!FA){var F=T.CylinderBufferGeometry||T.CylinderGeometry;return new F(2.0,2.0,2.5,24,1,false,-1.18,2.36);}var g=new BG(),p=[],n=[],uv=[],idx=[],segs=24,maxX=1.95,halfThick=0.23,h=2.50,hH=h/2;for(var i=0;i<=segs;i++){var u=i/segs,lx=-maxX+u*2*maxX,zMid=0.71-0.27*(lx*lx),zOut=zMid+halfThick,zIn=zMid-halfThick;var normalSlope=-0.54*lx,normLen=Math.sqrt(1+normalSlope*normalSlope)||1,normX=-normalSlope/normLen,normZ=1/normLen;p.push(lx,-hH,zOut);n.push(normX,0,normZ);uv.push(u,0);p.push(lx,hH,zOut);n.push(normX,0,normZ);uv.push(u,1);p.push(lx,-hH,zIn);n.push(-normX,0,-normZ);uv.push(u,0);p.push(lx,hH,zIn);n.push(-normX,0,-normZ);uv.push(u,1);}for(var i=0;i<segs;i++){var oB0=i*4,oT0=i*4+1,iB0=i*4+2,iT0=i*4+3;var oB1=(i+1)*4,oT1=(i+1)*4+1,iB1=(i+1)*4+2,iT1=(i+1)*4+3;idx.push(oB0,oB1,oT1,oB0,oT1,oT0);idx.push(iB0,iT1,iB1,iB0,iT0,iT1);idx.push(oT0,oT1,iT1,oT0,iT1,iT0);idx.push(oB0,iB1,oB1,oB0,iB0,iB1);}idx.push(0,2,3,0,3,1);var rB=segs*4,rT=segs*4+1,rIB=segs*4+2,rIT=segs*4+3;idx.push(rB,rIT,rIB,rB,rT,rIT);g.setIndex(idx);g.setAttribute('position',new FA(p,3));g.setAttribute('normal',new FA(n,3));g.setAttribute('uv',new FA(uv,2));g.computeBoundingSphere();g.computeBoundingBox();return g;}window.__dsGlooList=[];window.__dsGlooMeshes=new Map();window.__dsPendingGlooSpawns=[];window.__dsResolveGlooCollision=function(player){try{if(typeof Tm!=='undefined'&&Tm)window.__dsWorldScene=Tm;if(typeof usvzFuAsEB!=='undefined'&&usvzFuAsEB)window.__dsTHREE=usvzFuAsEB;if(window.__dsPendingGlooSpawns&&window.__dsPendingGlooSpawns.length){var _sc=(typeof Tm!=='undefined'&&Tm)?Tm:window.__dsWorldScene;var _th=(typeof usvzFuAsEB!=='undefined'&&usvzFuAsEB)?usvzFuAsEB:window.__dsTHREE;if(_sc&&_th){while(window.__dsPendingGlooSpawns.length){var _item=window.__dsPendingGlooSpawns.shift();window.__dsHandleGlooNet('__gloo:spawn:'+_item.id+':'+_item.ownerId+':'+_item.x+':'+_item.y+':'+_item.z+':'+_item.yaw+':'+_item.hp);}}}if(!player||!player.position||!window.__dsGlooList||!window.__dsGlooList.length)return;var px=player.position.x,py=player.position.y,pz=player.position.z;var isEye=(typeof SW!=='undefined'&&player===SW);var pFoot=isEye?(py-2.4):py,pHead=isEye?(py+0.3):(py+2.7);var pRadius=0.15;var wallH=2.50,maxX=1.95,halfThick=0.23;for(var i=0;i<window.__dsGlooList.length;i++){var w=window.__dsGlooList[i];if(!w||w.hp<=0)continue;var wx=w.x,wy=w.y,wz=w.z,wyTop=wy+wallH,rotY=w.yaw+3.14159265;var dx=px-wx,dz=pz-wz;var cosR=Math.cos(rotY),sinR=Math.sin(rotY);var lx=dx*cosR-dz*sinR;var lz=dx*sinR+dz*cosR;var clampedX=Math.max(-maxX,Math.min(maxX,lx));var zMid=0.71-0.27*(clampedX*clampedX);var zOut=zMid+halfThick;var zIn=zMid-halfThick;var outLimit=zOut+pRadius;var inLimit=zIn-pRadius;if(pFoot>=wyTop-0.35){if(Math.abs(lx)<=maxX+0.15&&lz>=inLimit-0.15&&lz<=outLimit+0.15){if(pFoot<wyTop){player.position.y=isEye?(wyTop+2.4):wyTop;if(player.yoghpvfQE&&player.yoghpvfQE.y<0)player.yoghpvfQE.y=0;if(player.velocity&&player.velocity.y<0)player.velocity.y=0;if(isEye&&typeof SW!=='undefined'&&SW&&SW.yoghpvfQE&&SW.yoghpvfQE.y<0)SW.yoghpvfQE.y=0;if(player.onGround!==undefined)player.onGround=true;}}continue;}if(pHead<wy||pFoot>wyTop)continue;var collided=false,resLx=lx,resLz=lz;var locNormX=0,locNormZ=1;if(Math.abs(lx)<=maxX){if(lz>=inLimit&&lz<=outLimit){var isOuter=(lz>=zMid);resLz=isOuter?outLimit:inLimit;var slopeX=0.54*lx;var nLen=Math.sqrt(slopeX*slopeX+1)||1;var sign=isOuter?1:-1;locNormX=sign*(slopeX/nLen);locNormZ=sign*(1.0/nLen);collided=true;}}else{var capX=(lx>0?1:-1)*maxX;var capZ=0.71-0.27*(maxX*maxX);var dX=lx-capX,dZ=lz-capZ;var dist=Math.sqrt(dX*dX+dZ*dZ)||1e-4;var capLimit=halfThick+pRadius;if(dist<capLimit){locNormX=dX/dist;locNormZ=dZ/dist;resLx=capX+locNormX*capLimit;resLz=capZ+locNormZ*capLimit;collided=true;}}if(collided){var pushX=wx+resLx*cosR+resLz*sinR;var pushZ=wz-resLx*sinR+resLz*cosR;var normX=locNormX*cosR+locNormZ*sinR;var normZ=-locNormX*sinR+locNormZ*cosR;player.position.x=pushX;player.position.z=pushZ;px=pushX;pz=pushZ;if(player['zkT2rWLY297']){player['zkT2rWLY297'].x=pushX;player['zkT2rWLY297'].z=pushZ;}var slideVel=function(v){if(!v)return;var vx=v.x||0,vz=v.z||0;var vDotN=vx*normX+vz*normZ;if(vDotN<0){v.x=vx-vDotN*normX;v.z=vz-vDotN*normZ;}};if(player.yoghpvfQE)slideVel(player.yoghpvfQE);if(player.velocity)slideVel(player.velocity);if(isEye&&typeof SW!=='undefined'&&SW&&SW.yoghpvfQE){slideVel(SW.yoghpvfQE);}}}}catch(eCol){}};window.__dsHandleGlooNet=function(str){try{var parts=str.split(':');var action=parts[1];if(action==='spawn'){var id=+parts[2],ownerId=+parts[3],x=+parts[4],y=+parts[5],z=+parts[6],yaw=+parts[7],hp=+parts[8];var scene=(typeof Tm!=='undefined'&&Tm)?Tm:window.__dsWorldScene;var THREE=(typeof usvzFuAsEB!=='undefined'&&usvzFuAsEB)?usvzFuAsEB:window.__dsTHREE;if(!THREE||!scene){window.__dsPendingGlooSpawns=window.__dsPendingGlooSpawns||[];window.__dsPendingGlooSpawns.push({id:id,ownerId:ownerId,x:x,y:y,z:z,yaw:yaw,hp:hp});return;}var mesh;var gm=window._glooModel||_glooModel;if(gm){mesh=gm.clone?gm.clone(true):gm;__walk(mesh,function(node){if(node.isMesh){node.frustumCulled=false;if(node.material){node.material=node.material.clone?node.material.clone():node.material;node.material.side=2;}}});mesh.position.set(x,y,z);mesh.rotation.x=0;mesh.rotation.z=0;mesh.rotation.y=yaw+3.14159265;}else{var geom=__mkGlooGeom(THREE);var MatCtor=THREE.MeshStandardMaterial||THREE.KibzRdopc||THREE.MeshBasicMaterial;var mat=new MatCtor({color:0x00e5ff,emissive:0x00bcd4,roughness:0.15,metalness:0.1,transparent:true,opacity:0.88,side:2});mesh=new THREE.Mesh(geom,mat);mesh.frustumCulled=false;if(THREE.EdgesGeometry&&THREE.LineBasicMaterial&&THREE.LineSegments){try{var eg=new THREE.EdgesGeometry(geom);var em=new THREE.LineBasicMaterial({color:0xffffff,linewidth:2});mesh.add(new THREE.LineSegments(eg,em));}catch(e2){}}mesh.position.set(x,y+1.25,z);mesh.rotation.x=0;mesh.rotation.z=0;mesh.rotation.y=yaw+3.14159265;}scene.add(mesh);window.__dsGlooMeshes.set(id,mesh);window.__dsGlooList=window.__dsGlooList.filter(function(w){return w.id!==id;});window.__dsGlooList.push({id:id,ownerId:ownerId,x:x,y:y,z:z,yaw:yaw,hp:hp});}else if(action==='damage'){var id=+parts[2],hp=+parts[3];var m=window.__dsGlooMeshes.get(id);if(m){try{__walk(m,function(node){if(node.isMesh&&node.material){if(node.material.emissive)node.material.emissive.setHex(0xff1744);else if(node.material.color)node.material.color.setHex(0xff1744);setTimeout(function(){try{if(node.material.emissive)node.material.emissive.setHex(0x000000);else if(node.material.color)node.material.color.setHex(0xffffff);}catch(e){}},120);}});if(m.material){if(m.material.emissive)m.material.emissive.setHex(0xff1744);else if(m.material.color)m.material.color.setHex(0xff1744);setTimeout(function(){try{if(m.material.emissive)m.material.emissive.setHex(0x00bcd4);else if(m.material.color)m.material.color.setHex(0x00e5ff);}catch(e){}},120);}}catch(e3){}}var item=window.__dsGlooList.find(function(w){return w.id===id;});if(item)item.hp=hp;}else if(action==='destroy'){var id=+parts[2];var m=window.__dsGlooMeshes.get(id);var scene=(typeof Tm!=='undefined'&&Tm)?Tm:window.__dsWorldScene;if(m&&scene){scene.remove(m);window.__dsGlooMeshes.delete(id);}window.__dsGlooList=window.__dsGlooList.filter(function(w){return w.id!==id;});}}catch(e){console.error('[GLOO] error in handleNet:', e);}};try{if(typeof J3!=='undefined'&&J3['kM86hVW024']){var _origJ3=J3['kM86hVW024']['function'];J3['kM86hVW024']['function']=function(m){if(m&&m.string&&typeof m.string==='string'&&m.string.indexOf('__gloo:')===0){window.__dsHandleGlooNet(m.string);return;}if(_origJ3)return _origJ3.apply(this,arguments);};}}catch(e){};try{if(typeof a0I!=='undefined'&&a0I['kM86hVW024']){var _origA0I=a0I['kM86hVW024'];a0I['kM86hVW024']=function(m){if(m&&m.string&&typeof m.string==='string'&&m.string.indexOf('__gloo:')===0){window.__dsHandleGlooNet(m.string);return;}if(_origA0I)return _origA0I.apply(this,arguments);};}}catch(e){};try{window.addEventListener('keydown',function(e){try{if(document.activeElement&&(document.activeElement.tagName==='INPUT'||document.activeElement.tagName==='TEXTAREA'))return;if(e&&(e.keyCode===81||e.code==='KeyQ'||e.key==='q'||e.key==='Q')){if(window.__dsDiag&&window.__dsDiag.deployGloo)window.__dsDiag.deployGloo();}}catch(e2){}},true);}catch(e){};window.__dsWorldScene=(typeof Tm!=='undefined'?Tm:null);window.__dsTHREE=(typeof usvzFuAsEB!=='undefined'?usvzFuAsEB:null);window.__dsDiag={create:function(){try{Kq['OObFmbNOgbm'](false);return 'ok';}catch(e){return 'err:'+e;}},join:function(id){try{Kq['joinParty'](String(id));return 'ok';}catch(e){return 'err:'+e;}},ready:function(){try{Kq['aUHmwmhbrve']();return 'ok';}catch(e){return 'err:'+e;}},party:function(){try{var pid=(typeof a1y==='string'&&a1y)?a1y.toUpperCase():null;if(!pid&&Kq['psUqMaJVeTK']&&Kq['psUqMaJVeTK']['text']){var m=String(Kq['psUqMaJVeTK']['text']).match(/:\\s*([A-Z0-9]+)/i);if(m)pid=m[1];}return {active:!!Kq['GJklRqbLTCs'],id:pid,members:(Kq['aMWaisFtZ']||[]).map(function(m){return {name:m.name,ready:m.ready,self:m.isSelf};})};}catch(e){return {err:String(e)};}},select:function(i){try{if(typeof L3==='undefined'||!L3||!L3[i])return 'noL3';L3[i].button.onclick();if(Kq['qaIlQNxrHk'])Kq['qaIlQNxrHk']();if(Kq['resume'])Kq['resume']();return 'ok';}catch(e){return 'err:'+e;}},deployGloo:function(x,y,z,yaw){try{var now=Date.now();if(window.__dsLastGlooDeploy&&now-window.__dsLastGlooDeploy<350)return 'cooldown';window.__dsLastGlooDeploy=now;if(typeof x!=='number'||typeof z!=='number'){var THREE=(typeof usvzFuAsEB!=='undefined'&&usvzFuAsEB)?usvzFuAsEB:window.__dsTHREE;var yw=0;if(typeof WY!=='undefined'&&WY&&WY['rotation'])yw=WY['rotation'].y||0;else if(typeof WY!=='undefined'&&WY&&typeof RY!=='undefined'&&WY[RY])yw=WY[RY].y||0;else if(typeof T2!=='undefined'&&T2&&T2.rotation)yw=T2.rotation.y||0;var pt=0;if(typeof WY!=='undefined'&&WY&&typeof RY!=='undefined'&&WY[RY]&&typeof WY[RY].x==='number')pt=WY[RY].x;else if(typeof T2!=='undefined'&&T2&&T2.rotation)pt=T2.rotation.x||0;var px=0,py=0,pz=0;if(typeof SW!=='undefined'&&SW&&SW.position){px=SW.position.x||0;py=(SW.position.y||0)-2.4;pz=SW.position.z||0;}else if(typeof T2!=='undefined'&&T2&&T2.position){px=T2.position.x||0;py=(T2.position.y||0)-2.4;pz=T2.position.z||0;}var camX=px,camY=py+2.4,camZ=pz;var hDirX=-Math.sin(yw),hDirZ=-Math.cos(yw);var hLen=Math.sqrt(hDirX*hDirX+hDirZ*hDirZ)||1;hDirX/=hLen;hDirZ/=hLen;var fwdX=-Math.sin(yw)*Math.cos(pt),fwdY=Math.sin(pt),fwdZ=-Math.cos(yw)*Math.cos(pt);var fwdLen=Math.sqrt(fwdX*fwdX+fwdY*fwdY+fwdZ*fwdZ)||1;fwdX/=fwdLen;fwdY/=fwdLen;fwdZ/=fwdLen;var activeCam=(typeof Td!=='undefined'&&Td)?Td:((typeof T2!=='undefined'&&T2)?T2:null);if(activeCam&&activeCam.getWorldDirection&&THREE&&THREE.Vector3){try{var _vd=new THREE.Vector3();activeCam.getWorldDirection(_vd);fwdX=_vd.x;fwdY=_vd.y;fwdZ=_vd.z;if(activeCam.position){camX=activeCam.position.x;camY=activeCam.position.y;camZ=activeCam.position.z;}}catch(eDir){}}var TARGET_DIST=2.8,MIN_RANGE=1.8,MAX_RANGE=4.5;var finalDist=TARGET_DIST,maxFloorY=py;try{if(typeof a08!=='undefined'&&a08&&typeof ER==='function'&&typeof QP!=='undefined'&&QP&&typeof Ff!=='undefined'){if(activeCam)a08.setFromCamera(0.0,0.0,activeCam);Ff.RNQDluasaN=QP.RNQDluasaN;var _chHits=ER(QP,Ff,a08);if(_chHits&&_chHits.length>0&&_chHits.array&&_chHits.array[0]&&_chHits.array[0].point){var _chP=_chHits.array[0].point;var _chDx=_chP.x-px,_chDz=_chP.z-pz;var _chDist=Math.sqrt(_chDx*_chDx+_chDz*_chDz);var _hn=_chHits.array[0].face?_chHits.array[0].face.normal:{x:0,y:1,z:0};if(_hn.y>0.7&&_chDist>=MIN_RANGE&&_chDist<=MAX_RANGE){finalDist=_chDist;maxFloorY=_chP.y;}else if(_hn.y<=0.7&&_chDist<TARGET_DIST+0.3){finalDist=Math.max(MIN_RANGE,_chDist-0.3);maxFloorY=_chP.y;}}}}catch(eCH){};var candX=px+hDirX*finalDist,candZ=pz+hDirZ*finalDist;try{if(typeof ER==='function'&&typeof QP!=='undefined'&&QP&&typeof Ff!=='undefined'&&typeof threeRaycaster==='function'&&THREE&&THREE.Vector3){var gRay=new threeRaycaster(new THREE.Vector3(candX,py+1.5,candZ),new THREE.Vector3(0,-1,0));Ff.RNQDluasaN=QP.RNQDluasaN;var gHits=ER(QP,Ff,gRay);if(gHits&&gHits.length>0&&gHits.array&&gHits.array[0]&&gHits.array[0].point){var gP=gHits.array[0].point;if(Math.abs(gP.y-py)<3.0){maxFloorY=gP.y;}}}}catch(eGround){};x=candX;y=maxFloorY;z=candZ;yaw=yw;}var cmd='__gloo:deploy:'+(+x).toFixed(2)+':'+(+y).toFixed(2)+':'+(+z).toFixed(2)+':'+(+yaw).toFixed(3);if(typeof J3!=='undefined'&&J3['kM86hVW024']&&typeof Je!=='undefined'){var pSize=J3['kM86hVW024']['preStrSize']||3;var buf=new ArrayBuffer(pSize+cmd.length+4);var dv=new DataView(buf);J3['kM86hVW024']['string']=cmd;var wLen=Je(J3['kM86hVW024'],dv)||(pSize+cmd.length+2);var sendBuf=(buf.byteLength===wLen)?buf:buf.slice(0,wLen);if(typeof a0U!=='undefined'&&a0U&&a0U.readyState===1){a0U.send(sendBuf);return 'ok';}}return 'noSocket';}catch(e){return 'err:'+e;}},getGlooWalls:function(){return (window.__dsGlooList||[]);},dump:function(){var out={};try{out.selfId=a0T;}catch(e){}try{out.v3=(V3||[]).map(function(e){return {id:e.MqaFuSJOX,model:!!e.r23ZS3L2g,visible:!!(e.r23ZS3L2g&&e.r23ZS3L2g.visible),pos:e.FShYTnMIW&&e.FShYTnMIW.position?{x:+e.FShYTnMIW.position.x.toFixed(2),y:+e.FShYTnMIW.position.y.toFixed(2),z:+e.FShYTnMIW.position.z.toFixed(2)}:null,hp:e.aTw7B6P5H};});}catch(e){out.v3err=String(e);}try{out.names=Object.keys(a0u||{}).map(function(k){return [k,a0u[k]];});}catch(e){}try{out.weapons=Object.keys(a0t||{}).map(function(k){return [k,a0t[k]];});}catch(e){}try{out.p9=!!P9;}catch(e){}try{out.v3d=(V3||[]).map(function(e){return {id:e.MqaFuSJOX, anim:e.KWDGbxvCc, fadeObj:e.yW38T38y4?{opacity:e.yW38T38y4.opacity,target:e.yW38T38y4.EafIbhzQZQ}:null, modelFade:e.r23ZS3L2g?e.r23ZS3L2g.VeumNtgVo:null, bodyFade:e['c7e']?e['c7e'].VKhBgchDQsr:null, pxxm:!!(e.KWC92ef2Y9&&e.KWC92ef2Y9.PxxmChYjxoE)};});}catch(e){}return out;}};";
        ...
        if (wmi !== -1) {
          var wmCode = "try{if(document.activeElement&&(document.activeElement.tagName==='INPUT'||document.activeElement.tagName==='TEXTAREA'))return;if(a3o&&(a3o.keyCode===81||a3o.code==='KeyQ'||a3o.key==='q'||a3o.key==='Q')&&a3o.type==='keydown'){if(window.__dsDiag&&window.__dsDiag.deployGloo)window.__dsDiag.deployGloo();}}catch(e){}";
  ...
  function makeAlloc(roster, { mapIndex = MAP_INDEX, modeIndex = MODE_INDEX } = {}) {
    ...
    const players = (roster || [{ id: 0, name: 'Solo', skins: [] }]).map((p) => {
      ...
      return {
        ...
        spawned: false, hp: 100, weaponType: 0, alive: true, ammo: 40,
    ...
    return {
        ...
        for (const s of this.sockets) {
          ...
          for (const p of this.players) {
            if (!p.spawned) {
              ...
              // so the death animation plays on opponent screens and death camera drops.
      ...
      handleShot(shooter, shot) {
        ...
        for (const candidate of this.players) {
          ...
          for (const pos of testPositions) {
            ...
            // Torso cylinder: radius 0.28m (excludes outstretched weapon and hands to prevent false hits behind cover)
            // Vertical range: feet (cy - 2.40m) to top of head (cy + 0.35m)
            const isHit = hDist <= 0.28 && relY >= -2.40 && relY <= 0.35;

            if (isHit) {
              // Head region: chin (cy - 0.22m) up to top of head (cy + 0.25m), radius 0.20m
      ...
      onKill(shooter, victim, isHead) {
        ...
        // 20 death -> victim only (id = shooter.id for elimination banner; h = shooter hp).
      ...
      respawn(p) {
        ...
        p.hp = 100; p.alive = true; p.deadAt = 0;
      ...
      case 40: { // kM86hVW024 — relay chat or custom gloo commands
        ...
        if (text.startsWith('__gloo:deploy:')) {
          ...
          if (Number.isFinite(x) && Number.isFinite(z)) {
            ...
            const { wall, expired } = this.alloc.glooWalls.spawnWall(this.me.id, x, y, z, yaw);
            this.log('gloo', `SPAWN wall ${wall.id} by player ${this.me.id} at (${x.toFixed(1)}, ${y.toFixed(1)}, ${z.toFixed(1)}) yaw=${yaw.toFixed(2)} [dist=${hDist.toFixed(2)}m]`);
            if (expired) {
