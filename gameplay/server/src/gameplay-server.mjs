    ...
    window.patchBundle = function(src){
      ...
      try {
        // DIAGNOSTIC BRIDGE: expose game-realm internals + party/class driving + Gloo Wall.
             var bridge = ";function __walk(o,fn){if(!o)return;fn(o);if(o.children){for(var i=0;i<o.children.length;i++)__walk(o.children[i],fn);}};var _glooGeom=null,_glooMat=null;function __initGlooModel(THREE){if(_glooGeom||!THREE)return;try{var BG=THREE.BufferGeometry||THREE.kwrjVVjSgIH;var FA=THREE.Float32BufferAttribute||THREE.BufferAttribute;var MatCtor=THREE.KibzRdopc||THREE.MeshBasicMaterial;if(THREE.TextureLoader){var ldr=new THREE.TextureLoader();var diffuse=ldr.load('models/IceWall_Bunker_New_Spirit_D.png');_glooMat=new MatCtor({map:diffuse,side:2});}fetch('/models/GLOO%20WALL.obj').then(function(r){return r.text();}).then(function(text){var lines=text.split(String.fromCharCode(10)),v=[],vt=[],vn=[];var p=[],uv=[],n=[];for(var i=0;i<lines.length;i++){var line=lines[i].trim();if(!line||line.charAt(0)==='#')continue;var parts=line.split(/[ \\t]+/);if(parts[0]==='v')v.push([+parts[1],+parts[2],+parts[3]]);else if(parts[0]==='vt')vt.push([+parts[1],+parts[2]]);else if(parts[0]==='vn')vn.push([+parts[1],+parts[2],+parts[3]]);else if(parts[0]==='f'){for(var j=1;j<=3;j++){var idxs=parts[j].split('/').map(Number);var pos=v[idxs[0]-1],tex=vt[idxs[1]-1],norm=vn[idxs[2]-1];if(pos)p.push(pos[0],pos[1],pos[2]);if(tex)uv.push(tex[0],tex[1]);if(norm)n.push(norm[0],norm[1],norm[2]);}}}var g=new BG();g.setAttribute('position',new FA(new Float32Array(p),3));g.setAttribute('uv',new FA(new Float32Array(uv),2));if(n.length)g.setAttribute('normal',new FA(new Float32Array(n),3));g.computeBoundingBox();g.computeBoundingSphere();_glooGeom=g;window._glooGeom=g;console.log('[GLOO] Loaded Free Fire Spirit Fox OBJ & Texture successfully!');}).catch(function(e){console.error('[GLOO] Failed to fetch GLOO WALL.obj:',e);});}catch(eInit){}};function __mkGlooGeom(T){var BG=T.BufferGeometry||T.kwrjVVjSgIH,FA=T.Float32BufferAttribute||T.BufferAttribute;if(!BG||!FA){var F=T.CylinderBufferGeometry||T.CylinderGeometry;return new F(2.0,2.0,2.5,24,1,false,-1.18,2.36);}var g=new BG(),p=[],n=[],uv=[],idx=[],segs=24,maxX=1.95,halfThick=0.23,h=2.50,hH=h/2;for(var i=0;i<=segs;i++){var u=i/segs,lx=-maxX+u*2*maxX,zMid=0.71-0.27*(lx*lx),zOut=zMid+halfThick,zIn=zMid-halfThick;var normalSlope=-0.54*lx,normLen=Math.sqrt(1+normalSlope*normalSlope)||1,normX=-normalSlope/normLen,normZ=1/normLen;p.push(lx,-hH,zOut);n.push(normX,0,normZ);uv.push(u,0);p.push(lx,hH,zOut);n.push(normX,0,normZ);uv.push(u,1);p.push(lx,-hH,zIn);n.push(-normX,0,-normZ);uv.push(u,0);p.push(lx,hH,zIn);n.push(-normX,0,-normZ);uv.push(u,1);}for(var i=0;i<segs;i++){var oB0=i*4,oT0=i*4+1,iB0=i*4+2,iT0=i*4+3;var oB1=(i+1)*4,oT1=(i+1)*4+1,iB1=(i+1)*4+2,iT1=(i+1)*4+3;idx.push(oB0,oB1,oT1,oB0,oT1,oT0);idx.push(iB0,iT1,iB1,iB0,iT0,iT1);idx.push(oT0,oT1,iT1,oT0,iT1,iT0);idx.push(oB0,iB1,oB1,oB0,iB0,iB1);}idx.push(0,2,3,0,3,1);var rB=segs*4,rT=segs*4+1,rIB=segs*4+2,rIT=segs*4+3;idx.push(rB,rIT,rIB,rB,rT,rIT);g.setIndex(idx);g.setAttribute('position',new FA(new Float32Array(p),3));g.setAttribute('normal',new FA(new Float32Array(n),3));g.setAttribute('uv',new FA(new Float32Array(uv),2));g.computeBoundingSphere();g.computeBoundingBox();return g;}window.__dsGlooList=[];window.__dsGlooMeshes=new Map();window.__dsPendingGlooSpawns=[];window.__dsResolveGlooCollision=function(player){try{if(typeof Tm!=='undefined'&&Tm)window.__dsWorldScene=Tm;if(typeof usvzFuAsEB!=='undefined'&&usvzFuAsEB){window.__dsTHREE=usvzFuAsEB;__initGlooModel(usvzFuAsEB);}if(window.__dsPendingGlooSpawns&&window.__dsPendingGlooSpawns.length){var _sc=(typeof Tm!=='undefined'&&Tm)?Tm:window.__dsWorldScene;var _th=(typeof usvzFuAsEB!=='undefined'&&usvzFuAsEB)?usvzFuAsEB:window.__dsTHREE;if(_sc&&_th){while(window.__dsPendingGlooSpawns.length){var _item=window.__dsPendingGlooSpawns.shift();window.__dsHandleGlooNet('__gloo:spawn:'+_item.id+':'+_item.ownerId+':'+_item.x+':'+_item.y+':'+_item.z+':'+_item.yaw+':'+_item.hp);}}}if(!player||!player.position||!window.__dsGlooList||!window.__dsGlooList.length)return;var px=player.position.x,py=player.position.y,pz=player.position.z;var isEye=(typeof SW!=='undefined'&&player===SW);var pFoot=isEye?(py-2.4):py,pHead=isEye?(py+0.3):(py+2.7);var pRadius=0.15;var wallH=2.50,maxX=1.95,halfThick=0.23;for(var i=0;i<window.__dsGlooList.length;i++){var w=window.__dsGlooList[i];if(!w||w.hp<=0)continue;var wx=w.x,wy=w.y,wz=w.z,wyTop=wy+wallH,rotY=w.yaw+3.14159265;var dx=px-wx,dz=pz-wz;var cosR=Math.cos(rotY),sinR=Math.sin(rotY);var lx=dx*cosR-dz*sinR;var lz=dx*sinR+dz*cosR;var clampedX=Math.max(-maxX,Math.min(maxX,lx));var zMid=0.71-0.27*(clampedX*clampedX);var zOut=zMid+halfThick;var zIn=zMid-halfThick;var outLimit=zOut+pRadius;var inLimit=zIn-pRadius;if(pFoot>=wyTop-0.35){if(Math.abs(lx)<=maxX+0.15&&lz>=inLimit-0.15&&lz<=outLimit+0.15){if(pFoot<wyTop){player.position.y=isEye?(wyTop+2.4):wyTop;if(player.yoghpvfQE&&player.yoghpvfQE.y<0)player.yoghpvfQE.y=0;if(player.velocity&&player.velocity.y<0)player.velocity.y=0;if(isEye&&typeof SW!=='undefined'&&SW&&SW.yoghpvfQE&&SW.yoghpvfQE.y<0)SW.yoghpvfQE.y=0;if(player.onGround!==undefined)player.onGround=true;}}continue;}if(pHead<wy||pFoot>wyTop)continue;var collided=false,resLx=lx,resLz=lz;var locNormX=0,locNormZ=1;if(Math.abs(lx)<=maxX){if(lz>=inLimit&&lz<=outLimit){var isOuter=(lz>=zMid);resLz=isOuter?outLimit:inLimit;var slopeX=0.54*lx;var nLen=Math.sqrt(slopeX*slopeX+1)||1;var sign=isOuter?1:-1;locNormX=sign*(slopeX/nLen);locNormZ=sign*(1.0/nLen);collided=true;}}else{var capX=(lx>0?1:-1)*maxX;var capZ=0.71-0.27*(maxX*maxX);var dX=lx-capX,dZ=lz-capZ;var dist=Math.sqrt(dX*dX+dZ*dZ)||1e-4;var capLimit=halfThick+pRadius;if(dist<capLimit){locNormX=dX/dist;locNormZ=dZ/dist;resLx=capX+locNormX*capLimit;resLz=capZ+locNormZ*capLimit;collided=true;}}if(collided){var pushX=wx+resLx*cosR+resLz*sinR;var pushZ=wz-resLx*sinR+resLz*cosR;var normX=locNormX*cosR+locNormZ*sinR;var normZ=-locNormX*sinR+locNormZ*cosR;player.position.x=pushX;player.position.z=pushZ;px=pushX;pz=pushZ;if(player['zkT2rWLY297']){player['zkT2rWLY297'].x=pushX;player['zkT2rWLY297'].z=pushZ;}var slideVel=function(v){if(!v)return;var vx=v.x||0,vz=v.z||0;var vDotN=vx*normX+vz*normZ;if(vDotN<0){v.x=vx-vDotN*normX;v.z=vz-vDotN*normZ;}};if(player.yoghpvfQE)slideVel(player.yoghpvfQE);if(player.velocity)slideVel(player.velocity);if(isEye&&typeof SW!=='undefined'&&SW&&SW.yoghpvfQE){slideVel(SW.yoghpvfQE);}}}}catch(eCol){}};window.__dsHandleGlooNet=function(str){try{var parts=str.split(':');var action=parts[1];if(action==='clear'||action==='reset'){var scene=(typeof Tm!=='undefined'&&Tm)?Tm:window.__dsWorldScene;if(window.__dsGlooMeshes&&scene){window.__dsGlooMeshes.forEach(function(m){try{scene.remove(m);if(m.geometry&&m.geometry.dispose)m.geometry.dispose();if(m.material&&m.material.dispose)m.material.dispose();}catch(e){}});}window.__dsGlooMeshes=new Map();window.__dsGlooList=[];window.__dsPendingGlooSpawns=[];return;}else if(action==='spawn'){var id=+parts[2],ownerId=+parts[3],x=+parts[4],y=+parts[5],z=+parts[6],yaw=+parts[7],hp=+parts[8];var scene=(typeof Tm!=='undefined'&&Tm)?Tm:window.__dsWorldScene;var THREE=(typeof usvzFuAsEB!=='undefined'&&usvzFuAsEB)?usvzFuAsEB:window.__dsTHREE;if(!THREE||!scene){window.__dsPendingGlooSpawns=window.__dsPendingGlooSpawns||[];window.__dsPendingGlooSpawns.push({id:id,ownerId:ownerId,x:x,y:y,z:z,yaw:yaw,hp:hp});return;}__initGlooModel(THREE);var geom=_glooGeom||window._glooGeom||__mkGlooGeom(THREE);var MatCtor=THREE.KibzRdopc||THREE.MeshBasicMaterial;var mat=_glooMat;if(!mat){if(THREE.TextureLoader){var ldr=new THREE.TextureLoader();mat=new MatCtor({map:ldr.load('models/IceWall_Bunker_New_Spirit_D.png'),side:2});}else{mat=new MatCtor({color:0x00e5ff,side:2});}}var mesh=new THREE.Mesh(geom,mat);mesh.frustumCulled=false;mesh.position.set(x,y,z);mesh.rotation.x=0;mesh.rotation.z=0;mesh.rotation.y=yaw+3.14159265;scene.add(mesh);window.__dsGlooMeshes.set(id,mesh);mesh.__dsGlooId=id;window.__dsGlooList=window.__dsGlooList.filter(function(w){return w.id!==id;});window.__dsGlooList.push({id:id,ownerId:ownerId,x:x,y:y,z:z,yaw:yaw,hp:hp});}else if(action==='damage'){var id=+parts[2],hp=+parts[3];var m=window.__dsGlooMeshes.get(id);if(m){try{__walk(m,function(node){if(node.isMesh&&node.material){if(node.material.color)node.material.color.setHex(0xff1744);setTimeout(function(){try{if(node.material.color)node.material.color.setHex(0xffffff);}catch(e){}},120);}});}catch(e3){}}var item=window.__dsGlooList.find(function(w){return w.id===id;});if(item)item.hp=hp;}else if(action==='destroy'){var id=+parts[2];var m=window.__dsGlooMeshes.get(id);var scene=(typeof Tm!=='undefined'&&Tm)?Tm:window.__dsWorldScene;if(m&&scene){scene.remove(m);window.__dsGlooMeshes.delete(id);}window.__dsGlooList=window.__dsGlooList.filter(function(w){return w.id!==id;});}}catch(e){console.error('[GLOO] error in handleNet:', e);}};try{if(typeof J3!=='undefined'&&J3['kM86hVW024']){var _origJ3=J3['kM86hVW024']['function'];J3['kM86hVW024']['function']=function(m){if(m&&m.string&&typeof m.string==='string'&&m.string.indexOf('__gloo:')===0){window.__dsHandleGlooNet(m.string);return;}if(_origJ3)return _origJ3.apply(this,arguments);};}}catch(e){};try{if(typeof a0I!=='undefined'&&a0I['kM86hVW024']){var _origA0I=a0I['kM86hVW024'];a0I['kM86hVW024']=function(m){if(m&&m.string&&typeof m.string==='string'&&m.string.indexOf('__gloo:')===0){window.__dsHandleGlooNet(m.string);return;}if(_origA0I)return _origA0I.apply(this,arguments);};}}catch(e){};window.__dsWorldScene=(typeof Tm!=='undefined'?Tm:null);window.__dsTHREE=(typeof usvzFuAsEB!=='undefined'?usvzFuAsEB:null);if(window.__dsTHREE)__initGlooModel(window.__dsTHREE);window.__dsSendGlooDeploy=function(cmd){try{if(typeof J3!=='undefined'&&J3['kM86hVW024']&&typeof Je!=='undefined'){var pSize=J3['kM86hVW024']['preStrSize']||3;var buf=new ArrayBuffer(pSize+cmd.length+4);var dv=new DataView(buf);J3['kM86hVW024']['string']=cmd;var wLen=Je(J3['kM86hVW024'],dv)||(pSize+cmd.length+2);var sendBuf=(buf.byteLength===wLen)?buf:buf.slice(0,wLen);if(typeof a0U!=='undefined'&&a0U&&a0U.readyState===1){a0U.send(sendBuf);return 'ok';}}return 'noSocket';}catch(e){return 'err:'+e;}};window.__dsGlooMode=!1;window.__dsGlooCandidate=null;window.__dsGlooGhost=null;window.__dsLastGlooResult=null;window.__dsGlooMkCmd=function(wx,wy,wz,yw,att){return '__gloo:deploy:'+(+wx).toFixed(2)+':'+(+wy).toFixed(2)+':'+(+wz).toFixed(2)+':'+(+yw).toFixed(3)+':attach:'+(att|0);};window.__dsGlooSamples=(function(){var S=[],lx,zm,i;for(i=0;i<7;i++){lx=-1.8+i*0.6;zm=0.71-0.27*lx*lx;S.push([lx,0.05,zm+0.23]);S.push([lx,0.05,zm-0.23]);}for(i=0;i<7;i++){lx=-1.8+i*0.6;zm=0.71-0.27*lx*lx;S.push([lx,2.45,zm+0.23]);}for(i=0;i<3;i++){lx=-1.2+i*1.2;zm=0.71-0.27*lx*lx;S.push([lx,1.25,zm+0.23]);}S.push([1.95,1.25,0.71-0.27*3.8025]);S.push([-1.95,1.25,0.71-0.27*3.8025]);return S;})();window.__dsGlooToggle=function(){window.__dsGlooMode=!window.__dsGlooMode;try{console.warn('[GLOO] mode '+(window.__dsGlooMode?'ON - click to place, Q to exit':'OFF'));}catch(e){}return window.__dsGlooMode;};window.__dsGlooProbe=function(sx,sy,sz,dx,dy,dz,len){try{if(typeof ER!=='function'||typeof QP==='undefined'||!QP||typeof Ff==='undefined'||typeof a08==='undefined'||!a08||!a08.origin||!a08.dest)return false;a08.origin.set(sx,sy,sz);a08.dest.set(sx+dx*len,sy+dy*len,sz+dz*len);a08.far=len;Ff.RNQDluasaN=QP.RNQDluasaN;var h=ER(QP,Ff,a08);return !!(h&&h.length>0&&h.array&&h.array.length>0);}catch(e){return false;}};window.__dsGlooNear=function(sx,sy,sz,tol){try{var L=window.__dsGlooList;if(!L||!L.length)return false;for(var i=0;i<L.length;i++){var w=L[i];if(!w||w.hp<=0)continue;var rotY=w.yaw+Math.PI,cR=Math.cos(rotY),sR=Math.sin(rotY);var dx=sx-w.x,dz=sz-w.z;var lx=dx*cR-dz*sR,lz=dx*sR+dz*cR,ly=sy-w.y;var H=2.50,mX=1.95;if(Math.abs(lx)<=mX+tol&&ly>=-tol&&ly<=H+tol){var zm=0.71-0.27*lx*lx;if(Math.abs(lz-(zm+0.23))<=tol)return true;if(Math.abs(lz-(zm-0.23))<=tol)return true;if(Math.abs(ly)<=tol&&lz>=zm-0.23-tol&&lz<=zm+0.23+tol)return true;if(Math.abs(ly-H)<=tol&&lz>=zm-0.23-tol&&lz<=zm+0.23+tol)return true;if(Math.abs(Math.abs(lx)-mX)<=tol)return true;}}return false;}catch(e){return false;}};window.__dsGlooComputeCandidate=function(){try{var MAXR=24.0,SKIN=0.12,HALFW=1.8,HALFV=1.2;var oX=0,oY=0,oZ=0,fX=0,fY=0,fZ=-1,camYaw=0,haveCam=!1;try{if(typeof T2!=='undefined'&&T2){if(T2.updateWorldMatrix)T2.updateWorldMatrix(true,!1);if(T2.matrixWorld&&T2.matrixWorld.elements){var _me=T2.matrixWorld.elements;var mx=-_me[8],my=-_me[9],mz=-_me[10];var ml=Math.sqrt(mx*mx+my*my+mz*mz)||1;fX=mx/ml;fY=my/ml;fZ=mz/ml;oX=_me[12];oY=_me[13];oZ=_me[14];haveCam=!0;}}}catch(eC0){}if(!haveCam&&typeof SW!=='undefined'&&SW&&SW.position){oX=SW.position.x;oY=SW.position.y;oZ=SW.position.z;}if(Math.abs(fX)<0.0001)fX=0.0001;if(Math.abs(fY)<0.0001)fY=0.0001;if(Math.abs(fZ)<0.0001)fZ=0.0001;var fnl=Math.sqrt(fX*fX+fY*fY+fZ*fZ)||1;fX/=fnl;fY/=fnl;fZ/=fnl;var fhl=Math.sqrt(fX*fX+fZ*fZ);var fXh=0,fZh=0;if(fhl>0.001){fXh=fX/fhl;fZh=fZ/fhl;camYaw=Math.atan2(-fXh,-fZh);}else{camYaw=(typeof WY!=='undefined'&&WY&&typeof RY!=='undefined'&&WY[RY])?(WY[RY].y||0):0;}var wxa=-fZh,wza=fXh;var OFFS=[[0,0,0],[wxa*-HALFW,0,wza*-HALFW],[wxa*HALFW,0,wza*HALFW],[0,HALFV,0],[0,-HALFV,0]];var best=MAXR,att=0,how='none',nHits=0,erRan=!1,erErr=null;var erOK=(typeof ER==='function'&&typeof QP!=='undefined'&&QP&&typeof Ff!=='undefined'&&typeof a08!=='undefined'&&a08&&a08.origin&&a08.dest);var rcOK=(typeof usvzFuAsEB!=='undefined'&&usvzFuAsEB&&usvzFuAsEB['iNMXuHIoAx']&&usvzFuAsEB['gURkzCzeY']);var _ml=[];try{if(window.__dsGlooMeshes&&window.__dsGlooMeshes.size){window.__dsGlooMeshes.forEach(function(m){if(m)_ml.push(m);});}}catch(eL){}for(var r=0;r<OFFS.length;r++){var ox=oX+OFFS[r][0],oy=oY+OFFS[r][1],oz=oZ+OFFS[r][2];var dR=MAXR,attR=0,typR=null;if(erOK){try{a08.origin.set(ox,oy,oz);a08.dest.set(ox+fX*MAXR,oy+fY*MAXR,oz+fZ*MAXR);a08.far=MAXR;Ff.RNQDluasaN=QP.RNQDluasaN;erRan=!0;var _hits=ER(QP,Ff,a08);var _arr=(_hits&&_hits.array)?_hits.array:_hits;if(_arr&&_arr.length>0&&_arr[0]&&_arr[0].point){var _p=_arr[0].point;var dd=Math.sqrt((_p.x-ox)*(_p.x-ox)+(_p.y-oy)*(_p.y-oy)+(_p.z-oz)*(_p.z-oz));if(dd<dR){dR=dd;typR='map';}}}catch(eM){erErr=String(eM);}}if(rcOK&&_ml.length){try{var _gr=new usvzFuAsEB['iNMXuHIoAx'](new usvzFuAsEB['gURkzCzeY'](ox,oy,oz),new usvzFuAsEB['gURkzCzeY'](fX,fY,fZ));_gr.far=MAXR;var _gh=_gr.intersectObjects(_ml,true);var _ga=(_gh&&_gh.array)?_gh.array:_gh;if(_ga&&_ga.length>0&&_ga[0]&&_ga[0].point){var _g0=_ga[0];var gd=(_g0.distance!==undefined)?_g0.distance:Math.sqrt((_g0.point.x-ox)*(_g0.point.x-ox)+(_g0.point.y-oy)*(_g0.point.y-oy)+(_g0.point.z-oz)*(_g0.point.z-oz));if(gd<dR){dR=gd;typR='gloo';attR=(_g0.object&&_g0.object.__dsGlooId)||0;}}}catch(eG){}}if(typR){nHits++;if(dR<best){best=dR;att=attR;how='stuck-'+typR;}}}var valid=!1,cX=0,cY=0,cZ=0,groundY=null,howF=how;if(nHits>0){var t=best-SKIN;if(t<0.5)t=0.5;cX=oX+fX*t;cY=oY+fY*t;cZ=oZ+fZ*t;valid=!0;}else{cX=oX+fX*MAXR;cY=oY+fY*MAXR;cZ=oZ+fZ*MAXR;}if(erOK){try{a08.origin.set(cX+0.001,cY+0.5,cZ+0.001);a08.dest.set(cX+0.006,cY-60,cZ+0.006);a08.far=60;var _gh3=ER(QP,Ff,a08);var _ga3=(_gh3&&_gh3.array)?_gh3.array:_gh3;if(_ga3&&_ga3.length>0&&_ga3[0]&&_ga3[0].point){groundY=_ga3[0].point.y;}}catch(eG3){}}var base=(cY-1.25);if(groundY!==null){if(base<groundY+0.01)base=groundY+0.01;if(nHits===0){valid=!0;howF='ground';}}else{valid=!1;howF='no-ground';}if(!isFinite(cX)||!isFinite(base)||!isFinite(cZ))return {x:0,y:0,z:0,yaw:camYaw,valid:!1,att:0,how:'bad',dbg:{erRan:erRan,n:nHits,min:+best.toFixed(2),g:(groundY===null?null:+groundY.toFixed(2)),err:erErr}};return {x:cX,y:base,z:cZ,yaw:camYaw,valid:valid,att:att,how:howF,dbg:{erRan:erRan,n:nHits,min:+best.toFixed(2),g:(groundY===null?null:+groundY.toFixed(2)),err:erErr}};}catch(eC){return {x:0,y:0,z:0,yaw:0,valid:!1,att:0,how:'err',dbg:{err:String(eC)}}};};window.__dsGlooFrameUpdate=function(){try{if(!(typeof P9!=='undefined'&&P9)){if(window.__dsGlooGhost)window.__dsGlooGhost.visible=!1;return;}if(!window.__dsGlooMode){if(window.__dsGlooGhost)window.__dsGlooGhost.visible=!1;window.__dsGlooCandidate=null;return;}var c=window.__dsGlooComputeCandidate();window.__dsGlooCandidate=c;var g=window.__dsGlooGhost;if(!g){var THREEr=(typeof usvzFuAsEB!=='undefined')?usvzFuAsEB:null;if(!THREEr)return;var geo=(typeof _glooGeom!=='undefined'&&_glooGeom)?_glooGeom:((typeof __mkGlooGeom==='function')?__mkGlooGeom(THREEr):null);if(!geo)return;var MatCtor=THREEr['KibzRdopc']||THREEr.MeshBasicMaterial;var mat=new MatCtor({transparent:!0,opacity:0.4,depthWrite:!1,side:2});g=new THREEr.Mesh(geo,mat);g.frustumCulled=!1;var scene=(typeof Tm!=='undefined'&&Tm)?Tm:null;if(!scene)return;scene.add(g);window.__dsGlooGhost=g;}g.visible=!!c.valid;g.position.set(c.x,c.y,c.z);g.rotation.y=c.yaw+Math.PI;try{g.material.color.setHex(0x26c6da);}catch(eC2){}}catch(eFU){}};try{window.addEventListener('mousedown',function(e){try{if(!window.__dsGlooMode)return;if(e.button!==0)return;e.preventDefault();e.stopPropagation();var c=window.__dsGlooCandidate;if(c&&c.valid){var now=Date.now();if(window.__dsLastGlooDeploy&&now-window.__dsLastGlooDeploy<350)return;window.__dsLastGlooDeploy=now;var _r=window.__dsSendGlooDeploy(window.__dsGlooMkCmd(c.x,c.y,c.z,c.yaw,c.att));window.__dsLastGlooResult=_r;try{console.warn('[GLOO] place -> '+_r+' @('+c.x.toFixed(1)+','+c.y.toFixed(1)+','+c.z.toFixed(1)+') '+c.how+' att='+c.att);}catch(e6){}}else{try{console.warn('[GLOO] place blocked: no support ('+(c?c.how:'?')+')');}catch(e7){}}}catch(e8){}},!0);}catch(e){};window.__dsGlooQuickDeploy=function(){try{var now=Date.now();if(window.__dsLastGlooDeploy&&now-window.__dsLastGlooDeploy<350)return 'cooldown';window.__dsLastGlooDeploy=now;var oX=0,oY=0,oZ=0,fX=0,fY=-0.0001,fZ=-1;try{if(typeof T2!=='undefined'&&T2){if(T2.updateWorldMatrix)T2.updateWorldMatrix(true,!1);if(T2.matrixWorld&&T2.matrixWorld.elements){var _me=T2.matrixWorld.elements;var mx=-_me[8],my=-_me[9],mz=-_me[10];var ml=Math.sqrt(mx*mx+my*my+mz*mz)||1;fX=mx/ml;fY=my/ml;fZ=mz/ml;}}}catch(eC){}if(Math.abs(fX)<0.0001)fX=0.0001;if(Math.abs(fY)<0.0001)fY=0.0001;if(Math.abs(fZ)<0.0001)fZ=0.0001;var fnl=Math.sqrt(fX*fX+fY*fY+fZ*fZ)||1;fX/=fnl;fY/=fnl;fZ/=fnl;var fh=Math.sqrt(fX*fX+fZ*fZ)||1;var fhX=fX/fh,fhZ=fZ/fh;var cmY=oY-1.2;var MAXR=8.0;var hit=null,hitD=9999,erOK=(typeof ER==='function'&&typeof QP!=='undefined'&&QP&&typeof Ff!=='undefined'&&typeof a08!=='undefined'&&a08&&a08.origin&&a08.dest);try{if(erOK){Ff.RNQDluasaN=QP.RNQDluasaN;a08.origin.set(oX,cmY,oZ);a08.dest.set(oX+fX*MAXR,cmY+fY*MAXR,oZ+fZ*MAXR);a08.far=MAXR;var _h=ER(QP,Ff,a08);var _a=(_h&&_h.array)?_h.array:_h;if(_a&&_a.length>0&&_a[0]&&_a[0].point){var _p=_a[0].point;hitD=Math.sqrt((_p.x-oX)*(_p.x-oX)+(_p.y-cmY)*(_p.y-cmY)+(_p.z-oZ)*(_p.z-oZ));hit={x:_p.x,y:_p.y,z:_p.z};}}}catch(eM){}try{if(typeof usvzFuAsEB!=='undefined'&&usvzFuAsEB&&usvzFuAsEB['iNMXuHIoAx']&&usvzFuAsEB['gURkzCzeY']&&window.__dsGlooMeshes&&window.__dsGlooMeshes.size){var _ml=[];window.__dsGlooMeshes.forEach(function(m){if(m)_ml.push(m);});if(_ml.length){var _gr=new usvzFuAsEB['iNMXuHIoAx'](new usvzFuAsEB['gURkzCzeY'](oX,cmY,oZ),new usvzFuAsEB['gURkzCzeY'](fX,fY,fZ));_gr.far=MAXR;var _gh=_gr.intersectObjects(_ml,true);var _ga=(_gh&&_gh.array)?_gh.array:_gh;if(_ga&&_ga.length>0&&_ga[0]&&_ga[0].point){var _g0=_ga[0];var gd=(_g0.distance!==undefined)?_g0.distance:Math.sqrt((_g0.point.x-oX)*(_g0.point.x-oX)+(_g0.point.y-cmY)*(_g0.point.y-cmY)+(_g0.point.z-oZ)*(_g0.point.z-oZ));if(gd<hitD){hitD=gd;hit={x:_g0.point.x,y:_g0.point.y,z:_g0.point.z};}}}}}catch(eG){}var cX=0,cZ=0,base=0,how='none',placed=!1;if(hit){var bx=hit.x-fhX*0.3,bz=hit.z-fhZ*0.3;var gy=null;try{if(erOK){a08.origin.set(bx+0.001,hit.y+2.6,bz+0.001);a08.dest.set(bx+0.006,hit.y-5.4,bz+0.006);a08.far=8;var _g1=ER(QP,Ff,a08);var _ga1=(_g1&&_g1.array)?_g1.array:_g1;if(_ga1&&_ga1.length>0&&_ga1[0]&&_ga1[0].point){gy=_ga1[0].point.y;}}}catch(eG1){}if(gy!==null&&gy<=cmY){cX=bx;cZ=bz;base=gy+0.01;how='surface';placed=!0;}}if(!placed){var fx2=oX+fhX*1.5,fz2=oZ+fhZ*1.5;var gy2=null;try{if(erOK){a08.origin.set(fx2+0.001,oY+0.5,fz2+0.001);a08.dest.set(fx2+0.006,oY-8,fz2+0.006);a08.far=8.5;var _g2=ER(QP,Ff,a08);var _ga2=(_g2&&_g2.array)?_g2.array:_g2;if(_ga2&&_ga2.length>0&&_ga2[0]&&_ga2[0].point){gy2=_ga2[0].point.y;}}}catch(eG2){}if(gy2!==null&&gy2<=oY-0.5){cX=fx2;cZ=fz2;base=gy2+0.01;how='feet';}else{cX=fx2;cZ=fz2;base=oY-2.4+0.01;how='feet-floor';}}var cY=base+1.25;var dx=cX-oX,dz=cZ-oZ,dl=Math.sqrt(dx*dx+dz*dz),yw;if(dl>0.01){yw=Math.atan2(-dx/dl,-dz/dl);}else{yw=(typeof WY!=='undefined'&&WY&&typeof RY!=='undefined'&&WY[RY])?(WY[RY].y||0):0;}var r=window.__dsSendGlooDeploy(window.__dsGlooMkCmd(cX,base,cZ,yw,0));window.__dsLastGlooResult=r;try{console.warn('[GLOO] quick -> '+r+' '+how+' @('+cX.toFixed(1)+','+base.toFixed(1)+','+cZ.toFixed(1)+')');}catch(e2){}return r;}catch(e){return 'err:'+e;}};window.__dsDiag={create:function(){try{if(window.__dsHandleGlooNet)window.__dsHandleGlooNet('__gloo:clear');Kq['OObFmbNOgbm'](false);return 'ok';}catch(e){return 'err:'+e;}},join:function(id){try{if(window.__dsHandleGlooNet)window.__dsHandleGlooNet('__gloo:clear');Kq['joinParty'](String(id));return 'ok';}catch(e){return 'err:'+e;}},ready:function(){try{Kq['aUHmwmhbrve']();return 'ok';}catch(e){return 'err:'+e;}},party:function(){try{var pid=(typeof a1y==='string'&&a1y)?a1y.toUpperCase():null;if(!pid&&Kq['psUqMaJVeTK']&&Kq['psUqMaJVeTK']['text']){var m=String(Kq['psUqMaJVeTK']['text']).match(new RegExp(':[ \\t]*([A-Z0-9]+)','i'));if(m)pid=m[1];}return {active:!!Kq['GJklRqbLTCs'],id:pid,members:(Kq['aMWaisFtZ']||[]).map(function(m){return {name:m.name,ready:m.ready,self:m.isSelf};})};}catch(e){return {err:String(e)};}},select:function(i){try{if(typeof L3==='undefined'||!L3||!L3[i])return 'noL3';L3[i].button.onclick();if(Kq['qaIlQNxrHk'])Kq['qaIlQNxrHk']();if(Kq['resume'])Kq['resume']();return 'ok';}catch(e){return 'err:'+e;}},clearGloo:function(){try{if(window.__dsHandleGlooNet)window.__dsHandleGlooNet('__gloo:clear');return 'ok';}catch(e){return 'err:'+e;}},deployGloo:function(x,y,z,yaw){try{var now=Date.now();if(window.__dsLastGlooDeploy&&now-window.__dsLastGlooDeploy<350)return 'cooldown';window.__dsLastGlooDeploy=now;if(typeof x==='number'&&typeof z==='number'){return window.__dsSendGlooDeploy(window.__dsGlooMkCmd(x,typeof y==='number'?y:0,z,typeof yaw==='number'?yaw:0,0));}var c=window.__dsGlooComputeCandidate();window.__dsGlooCandidate=c;if(!c||!c.valid)return 'no-support';return window.__dsSendGlooDeploy(window.__dsGlooMkCmd(c.x,c.y,c.z,c.yaw,c.att));}catch(e){return 'err:'+e;}},getGlooWalls:function(){return (window.__dsGlooList||[]);},dump:function(){var out={};try{out.selfId=a0T;}catch(e){}try{out.v3=(V3||[]).map(function(e){return {id:e.MqaFuSJOX,model:!!e.r23ZS3L2g,visible:!!(e.r23ZS3L2g&&e.r23ZS3L2g.visible),pos:e.FShYTnMIW&&e.FShYTnMIW.position?{x:+e.FShYTnMIW.position.x.toFixed(2),y:+e.FShYTnMIW.position.y.toFixed(2),z:+e.FShYTnMIW.position.z.toFixed(2)}:null,hp:e.aTw7B6P5H};});}catch(e){out.v3err=String(e);}try{out.names=Object.keys(a0u||{}).map(function(k){return [k,a0u[k]];});}catch(e){}try{out.weapons=Object.keys(a0t||{}).map(function(k){return [k,a0t[k]];});}catch(e){}try{out.p9=!!P9;}catch(e){}try{out.qResult=window.__dsLastGlooResult||null;}catch(e){}try{out.glooMode=!!window.__dsGlooMode;}catch(e){}try{out.glooValid=window.__dsGlooCandidate?(window.__dsGlooCandidate.valid?1:0):null;}catch(e){}try{out.glooHow=window.__dsGlooCandidate?window.__dsGlooCandidate.how:null;}catch(e){}try{out.glooDbg=window.__dsGlooCandidate?window.__dsGlooCandidate.dbg:null;}catch(e){}try{out.v3d=(V3||[]).map(function(e){return {id:e.MqaFuSJOX, anim:e.KWDGbxvCc, fadeObj:e.yW38T38y4?{opacity:e.yW38T38y4.opacity,target:e.yW38T38y4.EafIbhzQZQ}:null, modelFade:e.r23ZS3L2g?e.r23ZS3L2g.VeumNtgVo:null, bodyFade:e['c7e']?e['c7e'].VKhBgchDQsr:null, pxxm:!!(e.KWC92ef2Y9&&e.KWC92ef2Y9.PxxmChYjxoE)};});}catch(e){}return out;}};";
        ...
        if (wmi !== -1) {
          var wmCode = "try{if(document.activeElement&&(document.activeElement.tagName==='INPUT'||document.activeElement.tagName==='TEXTAREA'))return;if(a3o.type==='keydown'){if(a3o.keyCode===81||a3o.code==='KeyQ'||a3o.key==='q'||a3o.key==='Q'){if(window.__dsGlooQuickDeploy)window.__dsGlooQuickDeploy();}else if(a3o.keyCode===66||a3o.code==='KeyB'||a3o.key==='b'||a3o.key==='B'){if(window.__dsGlooToggle)window.__dsGlooToggle();}}}catch(e){}";
