// Debug: load the local game page and capture ALL network + console + errors.
...
const PORT = 9236;
const PROFILE = path.join(os.tmpdir(), 'ds-dbg-' + Date.now());
let chromeProc;
...
async function main() {
  ...
  const net = [];
  const consoleMsgs = [];
  cdp.on('Network.responseReceived', (p) => { if (net.length < 60) net.push(p.response.status + ' ' + p.type + ' ' + p.response.url); });
  cdp.on('Network.loadingFailed', (p) => { if (net.length < 60) net.push('FAIL ' + (p.blockedReason || p.errorText || '') + ' ' + p.requestId); });
  cdp.on('Network.webSocketCreated', (p) => { console.log('WS CREATED:', p.url); });
  cdp.on('Runtime.consoleAPICalled', (p) => { consoleMsgs.push((p.type === 'error' ? 'ERR: ' : '') + (p.args || []).map((a) => a.value || a.description || '').join(' ').slice(0, 150)); });
  cdp.on('Runtime.exceptionThrown', (p) => { consoleMsgs.push('EXC: ' + ((p.exceptionDetails.exception && p.exceptionDetails.exception.description) || p.exceptionDetails.text || '').slice(0, 200)); });
  ...
  console.log('\n=== NETWORK (first 60) ===');
  net.slice(0, 60).forEach((x) => console.log(' ', x));
  console.log('\n=== CONSOLE (first 30) ===');
  consoleMsgs.slice(0, 30).forEach((x) => console.log(' ', x));
  console.log('\n=== total: net=' + net.length + ' console=' + consoleMsgs.length + ' ===');
  fs.writeFileSync('raw/captures/local-page-debug.json', JSON.stringify({ net, consoleMsgs }, null, 2));
