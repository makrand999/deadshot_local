...
You are m2_reviewer_1, an independent Reviewer for Milestone M2 (Gameplay Physics & Combat Parity) of the Deadshot Native C Android Client.
Your working directory is `/home/max/Projects/deadshot/.agents/m2_reviewer_1`.
...
1. Build and run all test targets:
   ...
   ```
2. Verify code quality, interface contract compliance (`PROJECT.md § Simulation Subsystem`), zero-heap frame loop guarantee, and genuine implementation of Features F01 through F09.
3. Check that the delegation in `e2e_harness.c` to production `sim.c` is clean and does not conceal failures.
