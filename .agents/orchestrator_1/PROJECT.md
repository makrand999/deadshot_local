...
|---|---------|-------------|-----------|--------|
| F01 | 60Hz Physics & Kinematics | Rate-scaled physics loop, ground friction 0.76, air damping 0.95, jump impulses, gravity 0.036, terminal fall 0.72, crouch-slide | M2 | survey_gameplay_1 |
...
| M2 | Gameplay Physics & Combat Parity | 60Hz kinematics, collision, 4 weapons, recoil, classes, health, spectator | none | PLANNED |
| M3 | Renderer Frame Loop & HUD Integration | Hook viewmodel, remote players, tracers, HUD in android_main.c, immersive mode | M2 | PLANNED |
| M4 | Touch Input & Multi-Touch HUD Controls | Button bounding boxes, virtual joystick, touch look, zero-alloc input queue | M3 | PLANNED |
...
### Simulation Subsystem (`ds_sim.h`)
```c
typedef struct {
  float x, y, z;
  float vx, vy, vz;
  float yaw, pitch;
  int crouch, sprint, grounded;
  int weapon_idx;
  int ammo[4];
  int reserve[4];
  float reload_timer;
...
### Networking Subsystem (`ds_net.h`, `ds_discovery.h`, `ds_transport.h`)
```c
...
## Code Layout
- `android/native/include/ds/`: C headers (`ds_config.h`, `ds_audio.h`, `ds_sim.h`, `ds_mapgl.h`, `ds_net.h`, `ds_discovery.h`, `ds_transport.h`, `ds_hud.h`)
...
- `android/tests/`: Unit tests and E2E test harnesses (`test_all.c`, `test_audio.c`, `e2e/`)
